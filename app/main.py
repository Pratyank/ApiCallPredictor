...
async def predict_api_calls(request: PredictionRequest):
    ...
    try:
        ...
        if not safety_validator.validate_input(request.prompt):
            raise HTTPException(
                status_code=400,
                detail="Input failed safety validation")
        ...
        # Generate performance-optimized, safety-filtered, ML-ranked
        # predictions
        ...
        logger.info(
            f"Phase 5 request completed in {
                total_request_time:.2f}ms (target: <800ms)")
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance-optimized prediction failed: {str(e)}")
...
async def get_metrics():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Safety metrics unavailable")
...
async def generate_training_data():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Data generation failed: {
                str(e)}")
