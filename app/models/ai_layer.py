...
class AiLayer:
                ...
                with open(env_path) as f:
                    for line in f:
                        ...
                        if line and not line.startswith('#') and '=' in line:
                            ...
                            if not os.getenv(
                                    key):  # Don't override existing env vars
    ...
    async def _init_apis(self):
        ...
        if not self.anthropic_client:
            try:
                ...
                logger.info(
                    f"Anthropic API key from environment: {
                        'Found' if api_key else 'NOT FOUND'}")
                if api_key:
                    logger.info(
                        f"Anthropic API key length: {
                            len(api_key)} characters")
                ...
                logger.warning(
                    "Anthropic library not installed, will use fallback")
    ...
    async def _init_sentence_model(self):
        ...
        if not self.model_loaded:
            try:
                ...
                logger.error(
                    "sentence-transformers not installed. Please install it for semantic similarity features")
                self.model_loaded = False
            except Exception as e:
                logger.error(
    ...
    async def generate_predictions(
        ...
        try:
            ...
            logger.info(
                f"Generated {
                    len(final_predictions)} AI predictions in {
    ...
    def _extract_recent_events(
            self, history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Extract and format recent events from history"""
        if not history:
            return []

        # Take the most recent events (up to max_context_events)
        recent_events = history[-self.max_context_events:] if len(
            history) > self.max_context_events else history
        ...
        logger.debug(
            f"Extracted {
                len(normalized_events)} recent events from history")
    ...
    async def _get_available_endpoints(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {
                    len(endpoints)} available endpoints from cache")
            return endpoints
    ...
    async def _filter_endpoints_by_context(
        ...
        try:
            ...
            if recent_events:
                filtered_endpoints = self._filter_by_recent_patterns(
                    endpoints, recent_events)
            else:
            ...
            max_endpoints = 30
            final_filtered = filtered_endpoints[:max_endpoints] if len(
                filtered_endpoints) > max_endpoints else filtered_endpoints

            logger.debug(
                f"Filtered to {
                    len(final_filtered)} relevant endpoints")
    ...
    def _filter_by_recent_patterns(
        ...
        recent_methods = [event.get('method', '').upper()
                          for event in recent_events]
        recent_paths = [event.get('api_call', '') for event in recent_events]
        recent_resources = set()

        for path in recent_paths:
            # Extract resource names from paths (e.g., '/api/users/123' ->
            # 'users')
        ...
        for endpoint in endpoints:
            ...
            if isinstance(endpoint_tags, list):
                for tag in endpoint_tags:
                    if isinstance(
                            tag, str) and tag.lower() in recent_resources:
                        score += 1
        ...
        filtered = [
            endpoint for score,
            endpoint in scored_endpoints if score > 0]
        ...
        logger.debug(
            f"Pattern-based filtering: {len(filtered)} endpoints retained")
        return filtered

    async def _get_cached_embedding(
            self, text: str, model_name: str = 'all-MiniLM-L6-v2') -> Optional[np.ndarray]:
    ...
    async def _batch_encode_with_cache(self,
    ...
    async def _filter_by_semantic_similarity(
        ...
        try:
            ...
            filtered_endpoints = [endpoint for score,
                                  endpoint in similarity_scores[:top_k]]

            logger.debug(
                f"Semantic filtering: selected {
                    len(filtered_endpoints)} most similar endpoints (cached: {
                    len(
                        [
                            e for e in endpoint_embeddings if e is not None])})")
    ...
    async def _generate_ai_candidates(
        ...
        try:
            if self.anthropic_client:
                ...
                logger.warning(
                    "No AI providers available, using rule-based fallback")
...
Focus on realistic API calls that would logically follow the user's history and fulfill their stated intent."""
    ...
    async def _generate_rule_based_predictions(
        ...
        for endpoint in endpoints[:num_candidates *
                                  2]:  # Consider more than needed
    ...
    async def _apply_semantic_scoring(
        ...
        try:
            ...
            for i, candidate in enumerate(candidates):
                ...
                enhanced_confidence = (
                    0.7 * original_confidence) + (0.3 * semantic_score)
                candidate['confidence'] = min(enhanced_confidence, 1.0)
                candidate['semantic_similarity'] = semantic_score

            logger.debug(
                "Applied cached semantic similarity scoring to candidates")
            return candidates
...
async def get_ai_layer() -> AiLayer:
    ...
    if ai_layer is None:
        ai_layer = AiLayer()
        # Initialize APIs immediately to ensure environment variables are
        # loaded
