...
class MLRanker:
    ...
    async def train_ranker(self, training_data: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Training set: {len(X_train)} samples, {len(groups_train)} groups")
            logger.info(f"Validation set: {len(X_val)} samples, {len(groups_val)} groups")
            ...
            ndcg_score = self._calculate_ndcg(y_val, val_predictions, groups_val)
            
            self.training_stats = {
                'training_samples': len(training_data),
                'features_extracted': len(X_train[0]) if len(X_train) > 0 else 0,
            ...
            logger.info(f"MLRanker training completed - NDCG: {ndcg_score:.4f}, Time: {training_time:.2f}s")
    ...
    async def rank_predictions(
        ...
        try:
            ...
            for prediction in predictions[:k + buffer]:  # Only process k+buffer candidates
                # Extract ML features
            ...
            X_pred = self._prepare_prediction_features(candidates_with_features)
            ...
            ranked_candidates.sort(key=lambda x: x['ml_ranking_score'], reverse=True)
            
            # Add ranking metadata
            for i, prediction in enumerate(ranked_candidates):
                prediction['ml_rank'] = i + 1
                prediction['model_version'] = self.training_stats.get('model_version', 'unknown')
            
            logger.info(f"Ranked {len(ranked_candidates)} predictions using ML model")
    ...
    async def _prepare_training_features(self, training_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _prepare_ranking_dataset(self, features_data: List[Dict[str, Any]]) -> Tuple:
        ...
        for sequence_id, group_items in sequence_groups.items():
            ...
            for item in group_items:
                ...
                if feature_vector is not None:
                    ...
                    # Convert is_positive to ranking label (1 for positive, 0 for negative)
        ...
        train_group_idx, val_group_idx = train_test_split(group_indices, test_size=0.2, random_state=42)
        ...
        for i, group_size in enumerate(groups):
            if i in train_group_idx:
                train_indices.extend(range(train_start, train_start + group_size))
    ...
    def _extract_feature_vector(self, feature_item: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            categorical_features = {
                'last_endpoint_type': ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'NONE', 'UNKNOWN'],
                'last_resource': ['users', 'items', 'documents', 'products', 'orders', 'files', 'auth', 'none', 'unknown'],
                'endpoint_type': ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'UNKNOWN']
            }
    ...
    def _prepare_prediction_features(self, candidates_with_features: List[Dict[str, Any]]) -> np.ndarray:
        """Prepare features for prediction"""
        feature_vectors = []
        
        for candidate in candidates_with_features:
            feature_vector = self._extract_feature_vector(candidate['features'])
            if feature_vector is not None:
    ...
    def _simple_accuracy(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    async def load_model(self) -> bool:
        ...
        try:
            ...
            if model_data:
                ...
                logger.info(f"Loaded MLRanker model v{model_data['model_version']}")
                return True
    ...
    async def _fallback_ranking(self, predictions: List[Dict[str, Any]], k: int) -> List[Dict[str, Any]]:
        """Fallback ranking when ML model not available"""
        logger.info("Using fallback ranking (confidence-based)")
        
        # Sort by confidence score
        ranked = sorted(predictions, key=lambda x: x.get('confidence', 0.0), reverse=True)
    ...
    async def retrain_with_feedback(self, feedback_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Retrain model with user feedback data"""
        logger.info(f"Retraining MLRanker with {len(feedback_data)} feedback samples")
