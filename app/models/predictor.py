...
class Predictor:
    ...
    def __init__(self):
        ...
        # Additional candidates for ML ranking (k + buffer = 5 total)
        self.buffer = 2
        ...
        logger.info(
            "Initialized Phase 4 Predictor with AI + ML + Cold Start + Safety integration")
    ...
    def _init_cold_start_db(self):
        ...
        try:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                logger.info(
                    "Initialized popular_endpoints table with default safe endpoints")
    ...
    async def cold_start_predict(
        ...
        """
        try:
            # Case 1: If prompt exists, use semantic search on endpoint
            # descriptions
            if prompt and prompt.strip():
                predictions = await self._semantic_search_endpoints(prompt, k)
                if predictions:
                    logger.info(
                        f"Cold start: Generated {
                            len(predictions)} predictions using semantic search")
                    return predictions
            ...
            if spec:
                ...
                if predictions:
                    logger.info(
                        f"Cold start: Generated {
                            len(predictions)} predictions from OpenAPI spec")
                    return predictions
            ...
            logger.info(
                f"Cold start: Generated {
                    len(predictions)} popular safe endpoint predictions")
            return predictions
    ...
    async def _semantic_search_endpoints(
            self, prompt: str, k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not endpoints:
                logger.warning(
                    "No endpoints with descriptions found for semantic search")
                return []

            # Prepare texts for semantic comparison
            prompt_embedding = model.encode([prompt])
            descriptions = [f"{ep[2]} {ep[3]}" if ep[3] else ep[2]
                            for ep in endpoints]
            ...
            similarities = np.dot(
                prompt_embedding,
                description_embeddings.T).flatten()
    ...
    async def _extract_popular_from_spec(
            self, spec: Dict[str, Any], k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for path, methods in paths.items():
                for method, details in methods.items():
                    if method.lower() in [
                            'get', 'post'] and len(predictions) < k * 2:
                        description = details.get(
                            'summary', details.get(
                                'description', 'API endpoint'))

                        # Calculate confidence based on safety and common
                        # patterns
                        confidence = 0.5
                        if method.lower() == 'get':
                            confidence += 0.2
    ...
    async def _get_popular_safe_endpoints(
            self, k: int) -> List[Dict[str, Any]]:
    ...
    async def _get_basic_fallback_endpoints(
            self, k: int) -> List[Dict[str, Any]]:
    ...
    async def predict(
        ...
        try:
            ...
            cache_key = self._generate_cache_key(
                prompt, history, k, temperature, use_ml_ranking)
            ...
            # Phase 4: Cold start check - use cold start if no history
            # available
            if not history or len(history) == 0:
                logger.info(
                    "No history available, using cold start prediction")
                ...
                if cold_start_predictions:
                    ...
                    filtered_count = len(
                        cold_start_predictions) - len(safe_cold_start)
                    if filtered_count > 0:
                        self.safety_filtered_count += filtered_count
                        logger.info(
                            f"Safety filter removed {filtered_count} unsafe cold start predictions")

                    # Format cold start results to match standard prediction
                    # format
                    ...
                    result = {
                        ...
                        "confidence_scores": [
                            pred.get(
                                'confidence',
                                0.0) for pred in safe_cold_start],
                        ...
                        "metadata": {
                            ...
                            "cold_start_method": cold_start_predictions[0].get(
                                'cold_start_type',
                                'unknown') if cold_start_predictions else 'none',
                    ...
                    logger.info(
                        f"Generated {
                            len(
                                result['predictions'])} cold start predictions in {
            ...
            logger.info(
                f"AI Layer generated {
                    len(ai_predictions)} candidates (parallel processing)")

            if not ai_predictions:
                # If AI fails and no history, try cold start as final fallback
                if not history or len(history) == 0:
                    logger.info(
                        "AI predictions failed with no history, trying cold start fallback")
                    cold_start_predictions = await self.cold_start_predict(prompt=prompt, k=k)
                    if cold_start_predictions:
                        # Apply safety filtering to cold start fallback
                        # predictions
                        safe_fallback = [
                            pred for pred in cold_start_predictions
                            if self.safety_validator.is_safe(pred)
                        ...
                        filtered_count = len(
                            cold_start_predictions) - len(safe_fallback)
                        if filtered_count > 0:
                            self.safety_filtered_count += filtered_count
                            logger.info(
                                f"Safety filter removed {filtered_count} unsafe fallback predictions")
            ...
            if use_ml_ranking:
                ...
                if ranked_predictions:
                    ...
                    logger.info(
                        f"ML Ranker returned {
                            len(ranked_predictions)} ranked predictions")
                else:
            ...
            # Phase 4: Safety filtering - filter k+buffer candidates and return
            # k safe ones
            if ranked_predictions:
                ...
                filtered_count = len(ranked_predictions) - \
                    len(safe_predictions)
                if filtered_count > 0:
                    self.safety_filtered_count += filtered_count
                    logger.info(
                        f"Safety filter removed {filtered_count} unsafe predictions")

                # If we have fewer than k safe predictions, try to get more
                # from original candidates
                if len(safe_predictions) < k and len(
                        ai_predictions) > len(ranked_predictions):
                    # Check remaining AI predictions for safe ones
                    remaining_predictions = ai_predictions[len(
                        ranked_predictions):]
                    additional_safe = [
                    ...
                    # Add additional safe predictions until we reach k or run
                    # out
                    safe_predictions.extend(
                        additional_safe[:k - len(safe_predictions)])

                    if additional_safe:
                        logger.info(
                            f"Added {len(additional_safe[:k - len(safe_predictions)])} additional safe predictions from buffer")
                ...
                logger.info(
                    f"Safety filtering: {
                        ...
                        len(final_predictions)} safe predictions")
            else:
                final_predictions = []
            ...
            logger.info(
                f"Generated {len(result['predictions'])} ML-ranked predictions in {processing_time_ms:.2f}ms")
            return result
    ...
    async def _store_prediction_features(
        ...
        try:
            ...
            # Extract features for the top prediction (most likely to be
            # clicked)
    ...
    async def _get_cached_prediction(
        ...
        try:
            ...
            if cache_key in self.prediction_cache:
                ...
                if time.time() - \
                        cached_data["timestamp"] < self.settings.cache_ttl_seconds:
    ...
    async def _get_fallback_predictions(
        ...
        filtered_count = len(fallback_predictions) - \
            len(safe_fallback_predictions)
        ...
        return {
            ...
            "confidence_scores": [
                pred["confidence"] for pred in selected_predictions],
        ...
        try:
            ...
            return {
                ...
                "avg_usage_count": float(
                    stats[2]) if stats and stats[2] else 0.0,
...
async def train_ml_predictor() -> Dict[str, Any]:
    ...
    predictor = await get_predictor()
    # Force rebuild Sat Sep 27 12:52:11 AM EDT 2025
