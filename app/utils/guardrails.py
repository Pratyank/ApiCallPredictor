...
DESTRUCTIVE_PATTERNS = {
    # All DELETE operations are potentially destructive
...
def is_safe(endpoint: str, params: Dict[str, Any]
            = None, prompt: str = None) -> Tuple[bool, str]:
    """
    ...
    if any(keyword in path.lower()
           for keyword in ['admin', 'system', 'root', 'config']):
        if method in ['DELETE', 'PUT', 'PATCH']:
...
class SafetyValidator:
    ...
    def __init__(self):
        ...
        logger.info(
            "Initialized Safety Validator with content filtering and security checks")
    ...
    def validate_input(
        ...
        try:
            ...
            if not self._validate_length(prompt):
                logger.warning(
                    f"Prompt length validation failed: {
                        len(prompt)} chars")
            ...
            if additional_context and not self._validate_additional_context(
                    additional_context):
        ...
        try:
            ...
            for i, prediction in enumerate(predictions):
                ...
                if not self._validate_api_call_security(api_call):
                    warnings.append(
                        f"Prediction {i}: Security check failed for API call")
                    continue
                ...
                if not self._validate_parameters(params):
                    ...
                    prediction["parameters"] = self._sanitize_parameters(
                        params)
                ...
                if not self._validate_description_content(description):
                    warnings.append(
                        f"Prediction {i}: Description content filtered")
                    prediction["description"] = self._sanitize_description(
                        description)
            ...
            logger.info(
                f"Validated {
                    len(predictions)} predictions, filtered to {
    ...
    def _check_rate_limit(self, user_id: str) -> bool:
        ...
        if len(user_limits["requests"]) >= 60:
            ...
            logger.warning(
                f"Rate limit exceeded for user {user_hash}, blocked for 5 minutes")
            return False
    ...
    def _validate_prediction_structure(
            self, prediction: Dict[str, Any]) -> bool:
    ...
    def _validate_description_content(self, description: str) -> bool:
        ...
        return len(description) <= 500 and not self._contains_excessive_profanity(
            description)
    ...
    def _contains_excessive_profanity(self, text: str) -> bool:
        ...
        # PLACEHOLDER: In production, this would use a comprehensive profanity
        # filter
        basic_profanity = ["damn", "shit", "fuck", "bitch"]  # Very basic list
        text_lower = text.lower()

        profanity_count = sum(
            1 for word in basic_profanity if word in text_lower)
    ...
    def _load_blocked_patterns(self) -> List[str]:
        ...
        # Basic blocked patterns - in production this would be more
        # comprehensive
        return [
    ...
    def is_safe(self, prediction: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._is_destructive_operation(api_call, method):
                logger.debug(
                    f"Blocked destructive operation: {method} {api_call}")
                return False
...
def validate_api_safety(api_call: str, method: str,
