...
class TestPredictor:
    ...
    def mock_guardrails(self):
        ...
        guardrails.filter_safe_candidates = Mock(
    ...
    @pytest.mark.asyncio
    async def test_predict_with_empty_prompt(
            self, predictor, sample_openapi_spec):
    ...
    async def test_predict_with_invalid_spec(self, predictor):
        ...
        predictor.predict = AsyncMock(
            side_effect=ValueError("Invalid OpenAPI specification"))
    ...
    @pytest.mark.asyncio
    async def test_predict_guardrails_block(
            self, predictor, sample_openapi_spec):
    ...
    @pytest.mark.asyncio
    async def test_predict_timeout_handling(
            self, predictor, sample_openapi_spec):
        """Test handling of AI layer timeouts."""
        predictor.predict = AsyncMock(
            side_effect=asyncio.TimeoutError("AI layer timeout"))
    ...
    def test_prediction_count_limits(
            self, predictor, k_value, expected_length):
