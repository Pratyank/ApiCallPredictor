...
class TestSQLInjectionPrevention:
    ...
    @pytest.mark.asyncio
    async def test_sql_injection_in_prompt(self, async_client, sql_injection_payloads, sample_openapi_spec):
        ...
        for payload in sql_injection_payloads:
            ...
            if response.status_code == 200:
                # If processed, response should not contain SQL injection artifacts
                response_text = response.text.lower()
                dangerous_keywords = ["drop table", "delete from", "union select", "exec sp_"]
                
                for keyword in dangerous_keywords:
                    assert keyword not in response_text
    
    @pytest.mark.asyncio
    async def test_sql_injection_in_events(self, async_client, sql_injection_payloads, sample_openapi_spec):
        """Test SQL injection prevention in event data."""
        malicious_events = []
        for i, payload in enumerate(sql_injection_payloads[:5]):  # Test first 5 payloads
...
class TestXSSPrevention:
    ...
    @pytest.mark.asyncio
    async def test_xss_prevention_in_prompt(self, async_client, xss_payloads, sample_openapi_spec):
    ...
    @pytest.mark.asyncio
    async def test_response_content_type_security(self, async_client, sample_openapi_spec):
...
class TestCommandInjectionPrevention:
    ...
    @pytest.mark.asyncio
    async def test_command_injection_prevention(self, async_client, command_injection_payloads, sample_openapi_spec):
        ...
        for payload in command_injection_payloads:
            ...
            if response.status_code == 200:
                response_text = response.text.lower()
                dangerous_commands = ["rm -rf", "cat /etc/passwd", "whoami", "wget", "curl"]
...
class TestPathTraversalPrevention:
    ...
    @pytest.mark.asyncio
    async def test_path_traversal_prevention(self, async_client, path_traversal_payloads, sample_openapi_spec):
...
class TestRateLimiting:
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting_per_ip(self, async_client, sample_openapi_spec):
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting_bypass_attempts(self, async_client, sample_openapi_spec):
...
class TestInputValidationSecurity:
    ...
    async def test_oversized_payload_protection(self, async_client):
        ...
        assert response.status_code in [413, 422]  # Payload Too Large or Unprocessable Entity
    
    @pytest.mark.asyncio
    async def test_null_byte_injection(self, async_client, sample_openapi_spec):
    ...
    @pytest.mark.asyncio
    async def test_unicode_normalization_attacks(self, async_client, sample_openapi_spec):
        """Test Unicode normalization attack prevention."""
        unicode_payloads = [
            "adsᵒᶠᵗʷᵃʳᵉ",  # Homograph attack
            "аdmin",  # Cyrillic 'а' instead of Latin 'a'
            "../../etc/passwd",  # Using Unicode equivalents
            "script\u003Ealert('xss')\u003C/script\u003E",  # Encoded angle brackets
        ]
...
class TestAuthenticationSecurity:
    ...
    @pytest.mark.asyncio
    async def test_authentication_bypass_attempts(self, async_client, sample_openapi_spec):
        ...
        for headers in bypass_headers:
            ...
            # This depends on whether authentication is required for the predict endpoint
    ...
    async def test_session_fixation_prevention(self, async_client):
        ...
        if set_cookie_header:
            # If cookies are set, they should be different from the malicious ones
...
class TestSecurityHeaders:
    ...
    async def test_information_disclosure_prevention(self, async_client):
        ...
        debug_headers = ["X-Debug-Token", "X-Debug-Token-Link", "X-Symfony-Profiler-Token"]
...
class TestSecurityLogging:
    ...
    @pytest.mark.asyncio
    async def test_malicious_request_logging(self, async_client, sample_openapi_spec):
        ...
        with patch("app.utils.security_logger.log_security_event") as mock_logger:
            ...
            assert any("sql injection" in str(call).lower() for call in logged_calls)
    
    @pytest.mark.asyncio
    async def test_rate_limit_violation_logging(self, async_client, sample_openapi_spec):
